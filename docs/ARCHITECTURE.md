...
Qwen Q4/Q5 ship opt-in MTP speculative variants: the profile's `speculative` field makes `servermgmt` resolve, download and SHA-256-verify the pinned draft model (`model_catalog.QWEN27B_MTP_DRAFT`) and append `--spec-type draft-mtp --spec-draft-model`. Recovery of an MTP session first drops the draft at the same context, then interleaves lower contexts as MTP/plain pairs; `gpu.lower_memory_profiles` and the `_recovery_origin_mtp` intent marker own that order. See [mtp-profiles](design/mtp-profiles.md).
