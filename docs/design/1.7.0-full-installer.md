...
- Offline Backup remains separate. The existing QwenHarness-Offline-Backup directory was not modified and remains version 1.6.2.
...
The test is a relocated runtime check on the current Windows host, not a clean Windows VM installation. Models are intentionally excluded from both installer payloads. Historical offline backup files are not inputs or outputs of the Full build.
