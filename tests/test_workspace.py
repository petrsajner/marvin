...
class WorkspaceTests(unittest.TestCase):
    ...
    def test_attachment_payload_names_preview_and_reload(self):
        ...
        uploaded = self.client.post(f"/api/sessions/{sid}/attachments", files={"file": ("preview-λ.png", buffer.getvalue(), "image/png")}).json()
        self.client.post(f"/api/sessions/{sid}/actions/draft", json={"text": "draft", "attachments": [uploaded]}).raise_for_status()
        self.assertEqual(self.client.get(f"/api/sessions/{sid}").json()["draft"]["attachments"][0]["name"], "preview-λ.png")
        ...
        self.assertEqual(files[0]["name"], "preview-λ.png")
