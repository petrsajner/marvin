async (page) => {
  ...
  release();
  await page.waitForFunction(() => document.querySelector('.model-progress')?.textContent.includes('Loading model'));
  ...
  await page.getByText('Model is ready', { exact: true }).waitFor();
  ...
  await page.getByRole('combobox', { name: 'Thinking', exact: true }).selectOption('medium');
  await page.locator('.activity-feedback.pending').filter({ hasText: 'Saving settings' }).waitFor();
  saved();
